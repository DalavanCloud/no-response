...
module.exports = class NoResponse {
    ...
    if (isMarked && issueInfo.data.user.login === comment.user.login) {
      if (perform) {
        ...
        if (issueInfo.data.state === 'closed' && issueInfo.data.user.login !== issueInfo.data.closed_by.login) {
          this.github.issues.edit({owner, repo, number, state: 'open', milestone: this.config.reopenedMilestone})
        }
      } else {
  ...
  async close (issue) {
    ...
    if (perform) {
      ...
      return this.github.issues.edit(Object.assign({}, issue, {state: 'closed', milestone: this.config.staleMilestone}))
    } else {
